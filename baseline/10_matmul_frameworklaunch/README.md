## 概述
本样例基于MatmulCustom算子工程，介绍了单算子工程、单算子调用。

## 目录结构介绍
```
├── 10_matmul_frameworklaunch  // 使用框架调用的方式调用Matmul自定义算子。
...
### 2. 生成自定义算子工程，复制host和kernel实现并编译算子<a name="operatorcompile"></a>
  - 切换到msOpGen脚本install.sh所在目录
    ...
    cd ${git_clone_path}/samples/operator/ascendc/0_introduction/10_matmul_frameworklaunch
    ```
