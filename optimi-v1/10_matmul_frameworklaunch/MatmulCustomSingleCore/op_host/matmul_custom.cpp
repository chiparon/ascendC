...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    int32_t baseM = (M >= 4096) ? 128 : 256;
    int32_t baseN = (N >= 2048) ? 256 : 128;
    uint32_t forceBaseM = GetEnvU32("MATMUL_FORCE_BASE_M", 0U);
    uint32_t forceBaseN = GetEnvU32("MATMUL_FORCE_BASE_N", 0U);
    if (forceBaseM > 0U && forceBaseN > 0U) {
        baseM = static_cast<int32_t>(forceBaseM);
        baseN = static_cast<int32_t>(forceBaseN);
    }
