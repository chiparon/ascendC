...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, WholeReduceSumCustomTilingData tilingData)
    {
        ...
        const uint32_t rowsPerCore = this->rows / blockNum;
        const uint32_t remainRows = this->rows % blockNum;
        this->rowCount = rowsPerCore + (blockIdx < remainRows ? 1U : 0U);
        this->rowOffset = rowsPerCore * blockIdx + (blockIdx < remainRows ? blockIdx : remainRows);
        if (this->rowCount == 0) {
            this->isActive = false;
            return;
        }
        ...
        this->totalSizeAligned = this->colAligned * this->rowCount;
        uint32_t reducedSizeAligned = ceil_div(this->rowCount * sizeof(datatype), byteAlign) * byteAlign;
        xGm.SetGlobalBuffer((__gm__ datatype *)x + this->rowOffset * this->cols, this->rowCount * this->cols);
        yGm.SetGlobalBuffer((__gm__ datatype *)y + this->rowOffset, this->rowCount);
...
private:
    __aicore__ inline void CopyIn()
    {
        ...
        AscendC::DataCopyExtParams copyParams = {(uint16_t)this->rowCount, colBytes, 0, 0, 0};
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::WholeReduceSum<datatype, true>(yLocal, xLocal, this->cols, this->rowCount, 1, 1, srcStride);
    ...
    __aicore__ inline void CopyOut()
    {
        ...
        uint16_t reducedBytes = this->rowCount * sizeof(datatype);
